...
subprocess.check_call(["g++",'-g','-o','./.qtest.out',os.path.join(args.directory,args.directory+".cpp")])
...
for i in sorted([x for x in os.listdir(args.directory) if x.endswith(".in")], key=lambda x: int(re.findall(r"\d+",x)[0])):
    with open(os.path.join(args.directory, i)) as f, open(os.path.join(args.directory, i[:-2]+"out")) as g:
        ...
        print(i+" "+str(count(subprocess.check_output(["./.qtest.out"], universal_newlines=True, stderr=(None if args.verbose or args.special and i in args.special else subprocess.DEVNULL), input=f.read()).strip() == g.read().strip())))
